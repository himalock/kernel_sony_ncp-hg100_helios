...
static int rt5628_init_reg(struct snd_soc_codec *codec)
{
	int i;

	for (i = 0; i < RT5628_INIT_REG_NUM; i++)
	{
		snd_soc_write(codec, init_data[i].reg, init_data[i].def);
	}
...
static int rt5628_hweq_info(struct snd_kcontrol *kcontrol,
...
static int rt5628_hweq_get(struct snd_kcontrol *kcontrol,
...
static int rt5628_hweq_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
...
static void hp_depop_mode2(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, 0x3a, 0x0130, 0x0130);
	//snd_soc_update_bits(codec, 0x3c, 0x2000, 0x2000);
	snd_soc_update_bits(codec, 0x3c, 0x7FF8, 0x7FF8);
	snd_soc_update_bits(codec, 0x3e, 0x0600, 0x0600);
	snd_soc_update_bits(codec, 0x5e, 0x03E0, 0x03E0);
#endif
...
static int rt5628_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	snd_soc_write(codec, RT5628_AUDIO_DATA_CTRL, iface);
	return 0;
}
			...
			struct snd_soc_dai *dai)
{
	...
	struct rt5628_priv *rt5628 = snd_soc_codec_get_drvdata(codec);
	u16 iface = snd_soc_read(codec, RT5628_AUDIO_DATA_CTRL) & 0xfff3;
	...
	if (coeff >= 0)
		snd_soc_write(codec, RT5628_DAC_CLK_CTRL, coeff_div[coeff].regvalue);
...
static int rt5628_set_dai_pll(struct snd_soc_dai *codec_dai, int pll_id,int source,unsigned int freq_in, unsigned int freq_out)
{
	...
	if (RT5628_PLL_FR_MCLK == pll_id)
	{
		for (i = 0; i < ARRAY_SIZE(codec_master_pll_div); i ++)
		{
			...
			{
				snd_soc_write(codec, RT5628_GLOBAL_CLK_CTRL, 0x0000);    			/*PLL source from MCLK*/
				snd_soc_write(codec, RT5628_PLL_CTRL, codec_master_pll_div[i].regvalue);   	/*set pll code*/
				snd_soc_update_bits(codec, RT5628_PWR_MANAG_ADD2, 0x1000, 0x1000);        	/*enable pll power*/
				ret = 0;
			}
		}
	}
	else     /*slave mode*/
	{
		for (i = 0; i < ARRAY_SIZE(codec_slave_pll_div); i ++)
		{
			...
			{
				snd_soc_write(codec, RT5628_GLOBAL_CLK_CTRL, 0x4000);    			/*PLL source from BCLK*/
				snd_soc_write(codec, RT5628_PLL_CTRL, codec_slave_pll_div[i].regvalue);  	 /*set pll code*/
				snd_soc_update_bits(codec, RT5628_PWR_MANAG_ADD2, 0x1000, 0x1000);       	 /*enable pll power*/
				ret = 0;
			}
		}
	...
	snd_soc_update_bits(codec, RT5628_GLOBAL_CLK_CTRL, 0x8000, 0x8000);
...
static int rt5628_set_bias_level(struct snd_soc_codec *codec,
...
#endif
		snd_soc_update_bits(codec, 0x04, 0x8080, 0x8080);
		snd_soc_update_bits(codec, 0x3e, 0x8600, 0x0000);
		snd_soc_update_bits(codec, 0x3c, 0x7FF8, 0x0000);
		snd_soc_update_bits(codec, 0x3a, 0x0100, 0x0000);
		break;
...
static int rt5628_suspend(struct snd_soc_codec *codec)
{
	struct rt5628_priv *rt5628 = snd_soc_codec_get_drvdata(codec);
...
static int rt5628_resume(struct snd_soc_codec *codec)
{
	struct rt5628_priv *rt5628 = snd_soc_codec_get_drvdata(codec);
...
static int rt5628_probe(struct snd_soc_codec *codec)
{
...
static struct snd_soc_codec_driver soc_codec_dev_rt5628 = {
	.probe = 	rt5628_probe,
...
static ssize_t show_reg(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	int ret13, ret14, ret15, ret16, ret17, ret18, ret19, ret20, ret21, ret22, ret23;
	struct rt5628_priv *rt5628 = NULL;
				...
				const struct i2c_device_id *id)
{
	...
	return snd_soc_register_codec(&i2c->dev, &soc_codec_dev_rt5628,
			&rt5628_dai, 1);
}

static int rt5628_i2c_remove(struct i2c_client *client)
{
